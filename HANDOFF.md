...
- WeChat DevTools validation: targeted automation flow passed; full 29-route behavior verification is still not done.
...
- Full 29-route behavior verification.
...
- 29-route static file existence check passed.
...
- 仍未勾：GUI/真机/DevTools 相关项、29-route smoke、图片/付款凭证真机选择、付款闭环 GUI 证据、全系统入口矩阵 GUI 验收、登录/待审核固定画面 GUI 证据、owner/admin allowlist 实际值验证、customer 分享下单规则实机验收。
...
- BUG-009 remains: full 29-route GUI smoke test still needs to be re-executed before checking the final MVP gate.
...
- Use real workflow smoke as the MVP GUI gate; keep direct 29-route route-open results as diagnostics.
...
- Provide a working DevTools automation ticket/session, or manually assist with the 29-route GUI smoke test if automation becomes unstable.
