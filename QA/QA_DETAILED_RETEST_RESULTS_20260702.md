...
| 2026-07-03 CST | BUG-006 code fix | Static Pass / GUI Unverified | `pages/customerOrders/index.*`, `repositories/customerOrderRepository.js`, `cloudfunctions/businessData/index.js` | Order action UX now uses a bottom form panel for payment method/remark, confirmed amount/remark, and cancel reason; local/cloud repositories persist the fields. Full GUI payment closure still needs DevTools/device retest |
...
| FLOW-008 | 客戶聲明付款 | `QA_DETAILED_RETEST_PLAN_20260702.md` | Partial | Static code fix: `pages/customerOrders/index.*` | `pages/customerOrders/index.js`, action panel, `updatePaymentStatus` | Payment method/remark form and required validation are implemented; need GUI interaction evidence |
| FLOW-009 | 導遊確認收款 | `QA_DETAILED_RETEST_PLAN_20260702.md` | Partial | Static code fix: `pages/customerOrders/index.*`, repositories/cloud function | `CustomerOrderRepository`, payment confirmation permission, amount persistence | Confirm amount/remark form and persistence are implemented; need guide GUI confirmation evidence |
...
| BUG-006 | Partially Fixed | Partial | Static code fix: `pages/customerOrders/index.*`, `repositories/customerOrderRepository.js`, `cloudfunctions/businessData/index.js` | `pages/customerOrders/index.js`, `CustomerOrderService.updatePaymentStatus`, `paymentHistory` persistence | Need full GUI payment form/confirmation/history run |
