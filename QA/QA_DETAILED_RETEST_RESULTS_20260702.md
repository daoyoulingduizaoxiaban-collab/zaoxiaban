...
| 2026-07-03 CST | BUG-004 code fix | Static Pass / GUI Unverified | `pages/home/index.js`, `pages/setting/index.js`, `repositories/cloudBusinessRepository.js` | Save-mode copy now branches by formal OpenID, QA override, and mock OpenID state. Formal OpenID GUI copy still needs DevTools/device retest |
| 2026-07-03 CST | BUG-006 code fix | Static Pass / GUI Unverified | `pages/customerOrders/index.*`, `pages/customerOrders/edit/index.js`, `services/customerOrder/customerOrderService.js`, `repositories/customerOrderRepository.js`, `cloudfunctions/businessData/index.js` | Order action UX now uses a bottom form panel for payment method/remark/proof images, confirmed amount/remark, and cancel reason; service/local/cloud paths reject empty paid declarations and non-positive confirmation amounts; formal cloud paths reject direct temporary product/proof image persistence. Full GUI payment closure still needs DevTools/device retest |
| 2026-07-03 CST | BUG-002 product image retest | Static Pass / GUI Blocked | `npm run lint`, `git diff --check`, `sub-pages/product/add/index.*`, `services/product/productService.js`, `repositories/cloudBusinessRepository.js` | Latest commit `32c19bd` keeps formal image persistence on `uploadProductImages -> wx.cloud.uploadFile -> fileID`, adds local/QA temporary-image warning, choose-media unsupported/cancel handling, and duplicate-submit guard. Existing automation ports `9420` and `19512` still failed to connect, so actual `wx.chooseMedia` selection and reopen display remain unverified |
| 2026-07-03 CST | GUI-006 QA isolation entry fix | Static Pass / GUI Unverified | `pages/my/index.js`, `pages/my/index.wxml`, `pages/my/index.less`, `README.md` | My / QA Seed panel now has dedicated `切换导游并查看订单` and `切换客户并查看订单` buttons. Each applies QA role override and switches directly to `/pages/customerOrders/index`, reducing stale-tab/manual-navigation ambiguity. README source-of-truth/current-state copy was corrected. Actual GUI click-through and isolation screenshots still need DevTools/device retest |
...
| FLOW-003 | 商品圖片真實上傳 | `QA_DETAILED_RETEST_PLAN_20260702.md` | Partial / GUI Blocked | Static code inspection plus validation commands | `uploadProductImages`, cloud storage fileID, product image rendering | Formal upload path and UI guardrails are present; still need DevTools/media picker or true device evidence. Seed/HTTPS URL is not enough |
...
| FLOW-008 | 客戶聲明付款 | `QA_DETAILED_RETEST_PLAN_20260702.md` | Partial | Static code fix: `pages/customerOrders/index.*`, `CustomerOrderService`, repositories/cloud function | `pages/customerOrders/index.js`, action panel, `updatePaymentStatus` | Payment method/remark/proof form and required validation are implemented; need GUI interaction evidence |
| FLOW-009 | 導遊確認收款 | `QA_DETAILED_RETEST_PLAN_20260702.md` | Partial | Static code fix: `pages/customerOrders/index.*`, service/repositories/cloud function | `CustomerOrderRepository`, payment confirmation permission, amount persistence | Confirm amount/remark form, positive amount enforcement, and persistence are implemented; need guide GUI confirmation evidence |
| FLOW-010 | 付款歷史追溯 | `QA_DETAILED_RETEST_PLAN_20260702.md` | Partial | Static code fix: repository/cloud function payment fields | `paymentStatusHistory`, repository append order, UI rendering | Payment fields/history persistence path is implemented; need GUI/history evidence after payment flow |
| FLOW-011 | guide/customer 資料隔離 | `QA_DETAILED_RETEST_PLAN_20260702.md` | Partial | `QA/screenshots/2026-07-02-detailed-retest/role-scope/01_my_qa_seed_role_panel.png` plus static code check | `CustomerOrderRepository.listVisible`, auth profile role/meta, cloud permission checks | QA-only role switch panel is visible and role set is correct; dedicated guide/customer order-isolation buttons now switch role and open customer orders. Actual GUI click-through and guide/customer order isolation still need stable DevTools/device validation; formal OpenID isolation remains separate |
...
| PAGE-010 | `pages/customerOrders/index` | `switchTab` as guide/customer | Partial | Computer Use accessibility tree observed guide customer-order list | role scope, customer order repository | Guide text `当前身份：导游/领队` and orders visible; My page now provides dedicated guide/customer order-isolation buttons that switch role then open this tab. Data isolation still needs stable GUI/device screenshots |
...
| PAGE-022 | `pages/my/index` | `switchTab` | Partial | `QA/screenshots/2026-07-02-detailed-retest/role-scope/01_my_qa_seed_role_panel.png` | my page role/session rendering | My page and QA Seed role panel render. Dedicated guide/customer order-isolation buttons are implemented; actual click evidence still needs stable DevTools/device validation |
...
| BUG-002 | Partially Fixed | Partial / GUI Blocked | `npm run lint`, `git diff --check`, static inspection of `sub-pages/product/add/index.*`, `ProductService.create`, `uploadProductImages` | `sub-pages/product/add/index.ts`, `uploadProductImages`, cloud storage fileID persistence | Latest code handles unsupported/cancelled media selection, duplicate submit, local/QA warning, and formal cloud upload path. Need actual `wx.chooseMedia` / true device evidence and reopen display check before `Fixed - Verified` |
| BUG-003 | Fixed - Verified | Not Started | TBD | `sub-pages/groupOrder/add`, `GroupOrderService`, customer entry rendering | TBD |
| BUG-004 | Partially Fixed | Partial | Static code fix: `pages/home/index.js`, `pages/setting/index.js`, `repositories/cloudBusinessRepository.js` | `getSaveModeText`, `AuthService` session meta, page copy | Formal/QA copy branches are implemented; need formal OpenID and QA/mock GUI comparison |
| BUG-005 | Fixed - Verified | Not Started | TBD | `services/auth/roleScope.js`, login role selector, cloud allowlist env | TBD |
| BUG-006 | Partially Fixed | Partial | Static code fix: `pages/customerOrders/index.*`, `pages/customerOrders/edit/index.js`, `services/customerOrder/customerOrderService.js`, `repositories/customerOrderRepository.js`, `cloudfunctions/businessData/index.js` | `pages/customerOrders/index.js`, `CustomerOrderService.updatePaymentStatus`, `paymentHistory` persistence, `businessData` validation | Need full GUI payment proof/form/confirmation/history run |
