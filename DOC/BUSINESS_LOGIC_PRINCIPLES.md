...
#### 代下单订单的归属（OWNER 决策 14 细则，2026-08-27）

**代下单的订单不绑任何客户帐号，只有团主看得到。**

- 订单归属写**代下单的团主**，不写被代下单的那位客户。
- 被代下单的人**在自己的「我的订单」里看不到这笔**——他没登入、也可能根本没注册，这是刻意的，不是漏做。
- 「帮谁下的」只以**文字**记录：客户姓名 + 手机号，加上「这是代下单」的标记。**不做**手机号比对、不做帐号绑定、不做事后认领。
- 客户日后自己注册登入，这笔订单**也不会**自动出现在他名下。要让客户自己看得到，就请他走正常的分享连结自助下单。

> **为什么不绑**：绑定唯一可用的键是手机号，而微信登入拿不到手机号（`authLogin` 建号时 `phone` 是空字串），
> 现况手机号纯自填、只验格式、无所有权验证 → 一旦「填了号就自动认领」，任何人填别人的号就能继承那笔订单（含已确认收款的）。
> 不绑就没有这个攻击面，也不需要 `getPhoneNumber` 之类的额外授权。

**实作要注意**：订单归属既然写团主，团主若同时具 `customer` 角色，这些代下单的单会混进他自己的「我的订单」视角。
团主端必须靠**代下单标记**把它们与团主自己下的单区分开，不能只看归属欄位。
...
- **团主代客下单**：记录代下单团主、填入的客户姓名/手机号、所属团单（决策 14）。代下单痕迹不得抹去。
...
- [ ] **C-PROXY-ORDER 团主代客下单（落地 A6「代下单订单的归属」，OWNER 决策 14）**
  - 背景：现况**做不到代下单**。云端 `customerOrders.create` 的闸门是 `assertApprovedProfile(profile,['customer','owner','admin'])` 再加
    `if (!hasRole(profile,'customer') && !isOwnerOrAdmin(profile))` → 纯 `guide` 帐号直接被拒；且下单页 `pages/customerOrders/edit`
    的自写闸门只认 `CUSTOMER_ORDER_CREATE`。另外收单截止后一律挡下（`create` 查 `status !== OPEN`、`getShareAccessError` 查 `endAt`）。
  - **不做的事**（免得又被加回来）：不比对手机号、不绑客户帐号、不做待认领、不做事后认领、不接 `getPhoneNumber`。
    订单归属就写代下单的团主，客户端看不到——见 A6。
  - 改动：
    - `cloudfunctions/businessData/resources/customerOrders.js`：`create` 放行本团归属的 `guide`（限自己管理的团单，走 `canManageGroupOrder`）；
      代下单时略过收单截止与 shareToken 检查（团主例外）；写入代下单标记与代下单团主。
    - `pages/customerOrders/edit/*`：`proxy=1` 模式的闸门、必填栏与文案，见 `PAGE_MAP.md` §5。
    - 团主端各订单渲染处加代下单标记（四处，见 `PAGE_MAP.md` §5.2）。
    - 栏位见 `FIELD_DICT.md` §5「待开发栏位——代客下单」。
  - 判定：本团团主在自己团单代客下单，**收单截止后仍可下**；订单落库带代下单标记、代下单团主、客户姓名/手机；
    团主端四处渲染都看得到标记且与团主自己下的单区分得开；被代下单的人用自己微信登入**看不到**这笔（刻意）；
    非本团团主不能代下单（后端拒绝，不只前端隐藏）；A7 操作记录有对应一笔。
...
- [ ] **D-PROXY-ORDER**：① 本团团主在自己团单代客下单（填客户姓名+手机）→ 订单落库、团主端四处渲染都显示「代下单」标记，与团主自己下的单区分得开。
  ② 团单**收单截止后**团主仍可代下单；同一时间客户自助下单要被挡（前端禁用 + 后端拒绝）。
  ③ 被代下单的那个人（用该手机号注册的帐号）登入后「我的订单」**看不到**这笔——这是预期行为，不是 bug。
  ④ 负向：非本团团主代下单要被**后端**拒绝，不能只靠前端隐藏按钮。
