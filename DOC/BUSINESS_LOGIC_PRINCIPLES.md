...
- **代客下单**（替客户建一笔新订单；与「代登记**付款**」是两件事，别混）：团主可代客户提交订单，且**必须记录是帮谁下的**（OWNER 决策 14，2026-08-27）。与客户自助下单的差别：
  ...
  - ⚠️ **现况未实作**：云端 `customerOrders.create` 把 `customerOpenId` 写成下单者自己，且角色闸门要求下单者具 `customer` 角色，
    纯 `guide` 帐号根本无法下单。缺口与改法见 `DOC/稽核交办单.md` C5。
...
**绑定键与验证（⚠️ 两题待 OWNER 确认，未确认前不要动码）**

绑定唯一可用的键是**手机号**——微信登入只拿得到 openId，`authLogin` 建号时 `phone` 是空字串。由此产生两题：

1. **自动绑定的实际触发点不是「登入」，是「客户手上有了手机号」那一刻。** 现况手机号只能靠客户自己去「我的 → 账号资料」填，
   多数客户不会主动填 → 写成「登入后自动绑」会做出一个**几乎不会触发**的功能。要真的能触发，需要在登入或首次进入订单页时**主动要手机号**。
2. **冒领风险**：现况手机号是**纯自填、只验格式**（`users.js` 的 `/^1[3-9]\d{9}$/`），没有任何所有权验证。
   若照「填了号就自动认领」实作，**任何人填别人的手机号就能继承那笔订单**，含已付款、已确认收款的订单。
   → 建议：自动绑定**只接受微信「手机号快速验证」**（`getPhoneNumber`，拿到的是微信验证过的号码，全仓目前没用过）取得的号码；
   纯自填的号码**不触发**自动绑定。
