...
- **未登录（无 profile / 无有效 session）**：**导向登录页**（`pages/login/login`，带 `redirectTo` 或 `gate` 回跳参数），登录成功回原页，失败留登录页。**不得停在原页显示登录后才有的数据或空列表**。此类由**全站统一守卫**强制执行（`utils/authGuard.js` 在 `app.js` 顶层改写全局 `Page`，给每页 `onShow` 前置登录判定，未登录一律 reLaunch 到登录页；新增页面自动纳入，不必各页自写），仅登录页自身在白名单内。
