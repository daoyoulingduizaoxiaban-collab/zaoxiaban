...
### 0.0 ⚠️ 先搞清楚：Part A/B 写的是「目标态」，不一定是现况

**这是本文件最容易害人的地方。** Part A/B 描述「功能该长怎样」，其中一部分**还没实作**。
把没做的当成现况去改码，会做出错误判断。分辨方式只有一条，**不要靠猜**：

> **看到 `🚧 未实作 → C-XXX` 标记，就是还没做的**，`C-XXX` 是它在 Part C 的开发项编号。
> 没有这个标记的段落，才是已经落地的现况。

对应规矩（写文件的人必须遵守，否则下一个接手的人一定误判）：

1. 往 Part A/B 写任何**还没实作**的行为，**必须同时**在 Part C 开一个 `C-XXX` 开发项、在 Part D 开对应验收点，
   并在 Part A/B 那段标上 `🚧 未实作 → C-XXX`。**只写 Part A 不开 Part C 项＝制造陷阱。**
2. 实作完成后，把 Part A/B 的 `🚧` 标记拿掉、Part C/D 的 checkbox 打勾，同一次提交做完。
...
**目前带 `🚧` 的项**：`C-PROXY-ORDER`（团主代客下单 + 订单认领，决策 14）。

**进度与检视追踪**：白话进度、以及**全功能检视表**（逐模组对照本文件 A/B 走查「入口 / 操作 / 字段 / 角色 / 连动」）都在 `进度总览.md`。分工——本文件是「应该长怎样」的标准，检视表是「查了没 · 发现什么」的记录，两边靠章节号对应。

**工程规范**：本文件讲「功能**该长怎样**」；`DEVELOPMENT_GUIDE.md` 讲「**怎么改代码**」（架构分层、数据流、**地端云端双通铁律**、加/改功能的标准配方、权限模型落地、命名/提交/审查纪律、派 agent 边界与禁止事项）。**改代码前先看 DEVELOPMENT_GUIDE，避免自由发挥。**

**UI 契约**：本文件 A12 定「功能入口该放哪」、A13 定「未登录该导去哪」，属**业务层**；具体到「哪一页跳去哪、返回落在哪、带什么参数」看 `PAGE_MAP.md`，「同一实体有哪些字段、显示在哪些页」看 `FIELD_DICT.md`。**做表单/卡片/详情弹窗、动任何跳转前先查这两份**——同一份数据在多页各画一次、每次字段都不一样，是本项目最大宗的 BUG 来源。
...
3. 团主维护自己的**供应商实体**（供应商是数据，不是账号角色）。**商品不是独立实体**——开团时内嵌新增，只活在团单的 `productList` 里（决策 6）。
4. 团主选择商品并**开团**。
5. 客户从团单入口**下单**并声明付款。
...
这条闭环是**先收单、统计完才向供应商叫货**，不是先备货再卖。因此：

- **不设库存 / 名额上限**（OWNER 决策 13，2026-08-27）。团单与商品**都不做** stock / quota 栏位，
  下单不检查剩余量，也不做超卖防呆。要停止收单就由团主手动切「停止收单」，或等收单截止时间到。
- 不要再提案「加库存」「加限购数量」「加剩余名额显示」——与业务模式冲突，已定案不做。
...
- `customer` **客户 — 默认开放身份**。任何微信登录用户默认拥有，无需审核。**只能经团主分享链接进入某个团单**，浏览**该团在售商品**并下单、声明与更新自己订单的付款状态、查看自己订单。**不能自由浏览别的团主的商品或随意加入别人的团单**——看商品只发生在「团单详情客户视图 / 下单页」内联。客户底部 NAV 仅 `团单`（自己参与过的）+ `客户订单` + `我的`。
...
- ~~`products`~~ **已整个删除**（决策 6）。商品不是独立实体，没有 `products` 集合、没有商品管理页。商品只以快照形式存在于 `groupOrders.productList` 与下面的 `groupOrderProducts`。
...
- 从合法团单入口浏览本团商品并下单。**团单停止收单后仍可查看**（商品/自己订单），但**不能提交下单**（前端禁用+后端拒绝）。
  此限制**只针对客户自助下单**；团主代客下单不受收单截止限制，见下方「团主可做的操作」。
- **同一客户可在同一团单下多次下单，不做阻挡**（OWNER 决策 15，2026-08-27）。每次提交就是**一笔独立订单**，
  各自有自己的金额与付款状态，不合并、不覆盖、不提示「你已下过单」。
...
- 更新团单状态（进行中、停止收单、结束等）与客户订单状态；收单状态在**团单详情**内直接切换（开放收单 ⇄ 停止收单，二次确认）。
- **删除团单（软删）**：列表卡片操作，**双重确认**；若已收 < 应收（尚有未收款客户订单）第二次确认要醒目提醒。数据保留（deletedAt），列表不再显示。
- **复制团单**：从列表点「复制」→ **跳到开团页并带入**源团单内容（标题加「（副本）」、描述、时间、商品快照含价格档与图片），团主确认/修改后保存才建立新团单；shareToken 与二维码由系统对新团单重新生成，状态重置为开放收单。
- **代客下单**（替客户建一笔新订单；与「代登记**付款**」是两件事，别混）：团主可代客户提交订单，且**必须记录是帮谁下的**（OWNER 决策 14，2026-08-27）。与客户自助下单的差别：
  - **不受收单截止限制**：团单 `endAt` 过后、或状态已停止收单，团主仍可代下单（客户自助则被挡）。这是刻意保留的团主例外，不是漏挡。
  - 订单必须能看出**是代下单**、**代下单的团主是谁**、**帮的客户是谁**，不能只留一个客户姓名文字。
  - 🚧 **未实作** → 开发项 **`C-PROXY-ORDER`**（Part C · C5），验收点 `D-PROXY-ORDER`（Part D · D5）。**本节描述的是目标态，不是现况。**

#### 代下单订单的归属与认领（OWNER 决策 14 细则，2026-08-27）

代下单的订单**要绑到真实客户帐号**，让那位客户自己登入也看得到。分两种情形：

| 代下单当下 | 订单归属 | 客户看得到吗 |
| --- | --- | --- |
| 该客户**已注册**（手机号对得上既有用户） | 立刻绑：`customerOpenId` 写那位客户 | ✅ 立刻看得到 |
| 该客户**没注册过** | **不绑**，`customerOpenId` 留空，订单标记为**待认领**，暂由代下单的团主负责 | 尚看不到 |

- **待认领订单在客户注册后自动绑定**：该手机号的用户日后登入并完成手机号绑定，系统自动把所有该号码的待认领订单挂到他名下，不需团主或客户手动操作。
- 无论绑没绑，订单都必须留着**代下单团主是谁**——绑定后也不能把这个痕迹洗掉（审计需要，见 A7）。
- 待认领订单在团主端**照常显示与操作**（确认收款、取消都不受影响），认领与否不影响金流。

**绑定键是手机号**——微信登入只拿得到 openId，`authLogin` 建号时 `phone` 是空字串，手机号是唯一能在客户注册后对上号的键。
由此有**两题待 OWNER 确认**（自动绑定的触发点、防冒领要不要强制微信验证号码），题目与建议写在开发项 `C-PROXY-ORDER`，**没答案前不要动码**。
...
- **团主代客下单**（记录代下单团主、填入的客户姓名/手机号、当下有没有绑成功），以及**待认领订单被自动绑定**的那一刻（记录绑到哪个 principal、依据哪个手机号）。绑定后不得抹去代下单痕迹（决策 14）。
...
### A9.1 操作结果反馈（增删改 → 统一模块 `utils/feedback.js`）

增删改的**操作结果提示全站统一**，不得各页各写、措辞不一，也不得把「保存模式」文案当成功提示。

- **标准文案**（按动作，`RESULT_TEXT`）：新建→「新增成功」；编辑 / 保存→「保存成功」；状态变更（上下架、停用⇄启用、确认收款、取消、审核）→「更新成功」；删除 / 软删→「删除成功」；提交申请（团主申请、意见反馈）→「提交成功」。
- **成功**用 `toastSuccess(text)`（绿色对勾 `icon:success`）；**失败**用 `toastError(res.error)`（无图标，优先后端具体错误，缺省「操作失败，请稍后重试」）。
- **「保存模式」文案 ≠ 操作结果**：`getSaveModeText`（资料已同步 / 资料已保存）只指示数据落到云端还是本地，**不得**作为操作成功提示（曾出现「新建团单成功却弹『资料已同步』」的问题，见 A9 不暴露同步实现细节）。若确需展示落库位置，另作信息条，不占用结果提示。
- **不在此标准内**（保留各自具体文案）：复制到剪贴板、生成二维码等一次性动作、表单内即时加项、字段校验警告、登录态失效提示——它们不是增删改结果。
...
- 团主能开团，**商品在开团页内嵌新增**（见 B2 #8），并能维护供应商实体。
...
## A13. 页面进入鉴权闸门（未登录 / 无权 / 停用的统一去向）

每个需要登录才能使用的页面，进入时（`onLoad`/`onShow` 取数前）必须**先判定登录与权限状态，再决定统一去向**，不得先把页面渲染成业务态、也不得停在原页显示空白 / 异常 / 不属于当前身份的业务数据。这是**前端体验闸门**，与 A4 后端权限是两层：前端闸门决定「把用户带去哪个正式画面」，A4 后端仍对每次读写做最终判定，前端隐藏不替代后端拒绝。

判定结果与统一去向（四类，缺一不可）：

- **未登录（无 profile / 无有效 session）**：**导向登录页**（`pages/login/login`，带 `redirectTo` 或 `gate` 回跳参数），登录成功回原页，失败留登录页。**不得停在原页显示登录后才有的数据或空列表**。
- **已登录但无该功能权限**（角色不符 / 团主功能但未持 guide）：留在原页显示**正式受限状态**（`无权限` 文案 + 引导），不导登录页、不显示他人数据。
- **已登录但账户停用 / 角色过期**：显示正式**停用 / 过期状态**并引导联系管理员；停用账户前端导向安全状态、后端拒绝一切读写（见 A3）。
- **数据取失败**：走 A11 的 `error` 态（失败文案 + 重试），不得白屏或停在加载中。

**公开可浏览的例外（明确白名单）**：只有以下入口允许**未登录 / 游客部分浏览**，且仅浏览、任何管理与写操作仍按权限隐藏 + 后端拒绝：

- **团单详情客户视图 / 下单页**：客户经团主分享链接进入，可浏览本团在售商品并走登录后下单（见 A6、C-SHARE）。管理操作（编辑团单 / 收款 / 全团统计）按 `canManageGroupOrder` 隐藏。
- 除白名单外的所有需登录页面（团单列表、商品管理、客户订单、供应商、用户审核、操作记录、数据中心、消息、个人资料等），未登录一律走上面「未登录 → 导向登录页」。

**一致性要求**：以上四类去向与白名单，全站页面必须走**同一套判定与跳转约定**（收口到共享脚手架，见 `behaviors/useAccessPage` 与 C-AUTH-GATE），不得各页各写、有的导登录页有的却停在原页显示空数据。

## A14. 用户目录与用户管理（owner/admin）

owner/admin 必须有一个正式的**用户目录（管理面）**，能**看到全部用户并管理任意一个**，而不是只能处理「待审核团主申请」队列。这是 B1 `userReview` / B5「用户目录 · 改他人资料·角色」的落地要求，与「团主申请审核」是**同一管理域的两个视图**，不得只做申请队列而缺全量目录。

### 目录必须能看到（每位用户）

- 显示名 / 头像、脱敏必要联系资料。
- **真实 `roles[]`**（不是单一 `displayRole` 文字标签）与 primary role、UI 角色文案。
- 账户状态 `reviewStatus`（approved / disabled / rejected）。
- 团主角色期限 `roleExpiresAt`（未设=不限期）、是否已过期。
- 团主申请状态 `guideApplication.status`（none/pending/approved/rejected）。

### 目录必须能管理（对任意非 owner 用户，不限于 pending）

- **改角色**：按 A2「追加不覆盖」调整 `roles[]`（如授予/收回 guide），保留客户基线，绝不把 `[customer,guide]` 冲成 `[guide]`。
- **停用 / 恢复**：把 `reviewStatus` 在 approved ⇄ disabled 间切换。
- **设 / 续 / 清角色期限**：用日期选择器设 `roleExpiresAt`。
- **审核团主申请**：通过 / 拒绝 pending 申请（并入本域，复用 C-REVIEW 的防误操作）。
...
| `customerOrderCreate` 下单 | 下单流程 | ✅ | ✅ 自助：经分享作客户时<br>✅ **代客下单**：见 A6 决策 14（⚠️ 未实作） | ✅ |
...
- **客户（纯 customer，不具开团能力）**：**我的订单 / 我的**——不显示「团单」tab（客户经分享/扫码直达团单，不需要列表入口）；「客户订单」tab 对客户显示为**「我的订单」**（看自己下了什么单），页面标题同步。
- **团主**（含 customer+guide）：团单 / 客户订单 / 我的
- **管理层**：全部 + **用户审核 tab**（仅 owner/admin 显示；「我的」页不再放重复入口）
- **⚠ #8 没有商品库，商品只活在团单里（OWNER 2026-08-26 最终定案，不要再问）**

  **开团时同步新增商品，全站没有任何独立的商品管理页面。**

  - 开团页填「商品名称 + 起订量 + 这一档总价 + 商品图（选填，最多 3 张）」，**直接加进本团单**。
  - 商品**不是独立实体**，没有自己的集合、列表、详情、上下架、软删除。它只是团单文件里的 `productList`，随团单一起生老病死。
  - 团单已建立后要改商品，去「编辑团单」；要看本团有哪些商品，去团单详情的「本团商品」。
  - 客户看商品只发生在「团单详情客户视图 / 下单页」，看的也是那张团单自己的 `productList`。

  **已整个删除**（不是隐藏、不是保留待拆回）：商品库页、商品浏览页、商品表单页、选品页、`ProductService`、`ProductRepository`、云端 `products` 资源与 `products` 集合、`FEATURE_KEYS.PRODUCTS` / `PRODUCT_MANAGE`、底部导航的商品库 tab、搜索页的商品分类。

  供应商实体保留（团主自己维护的上游名单），入口在「我的」。
...
**停用/删除（软删除，保留快照）**：供应商实体只做**停用 / 软删除**，不做硬删除。已建团单与订单已存的供应商信息是**快照**，停用/软删除后历史订单追溯不受影响；停用/软删除后的供应商**新团单不可再选用**（与商品软删除、C-GROUP-PRODUCT 一致）。
...
| 团单 | **底部 NAV** | ✅(自己相关) | ✅ | ✅ |
| 客户订单 | **底部 NAV** | ✅(自己的) | ✅(本团) | ✅ |
| ~~商品库（浏览/管理）~~ | **整个删除(#8)**，商品只在开团页内嵌新增、随团单存活 | — | — | — |
| 我的 | **底部 NAV** | ✅ | ✅ | ✅ |
| 个人资料 / 设置 | 我的·账号区 | ✅ | ✅ | ✅ |
| 操作记录 | 我的·账号区 | — | ✅(自己相关) | ✅(管理记录) |
| 申请成为团主 | 我的·账号区 | ✅(未持 guide 时) | — | — |
| 用户审核 | 我的·管理区 | — | — | ✅ |
| 供应商实体维护 | 我的·管理区（商品库删除后搬回来） | — | ✅ | ✅ |
| ~~用户目录 / 改他人资料·角色~~ | **已整页删除（决策 5）**，功能并入用户审核 | — | — | — |
| ~~工作台/首页(home)~~ | **废弃** | — | — | — |
| ~~开团入口页(release)~~ | **废弃**（开团走团单列表 FAB / 我的·常用工作） | — | — | — |
| ~~验证码登录(loginCode)~~ | **废弃**（登录统一走微信登录 login） | — | — | — |
| ~~聊天/订单公告(chat)~~ | **废弃**（A6：无聊天入口；消息页=订单提醒即可） | — | — | — |
...
**数据中心**（`pages/dataCenter`，入口 `DATA_CENTER` 特性）指标沿用现有口径，不额外扩充：

- 指标：进行中/团单数、客户订单数、待确认收款数、已确认收款数，以及应收总额（未取消订单合计）、已收总额（已确认订单的实收 `confirmedAmount` 合计）。
- 可见范围：**团主只看自己团单的汇总**（走各自 `listVisible` 的归属收敛）；**owner/admin 看全局**；**客户无数据中心权限**。
- 金额与订单口径与团单详情 / 客户订单模块一致，不新开另一套统计源。

**搜索**（`pages/search`）：搜索结果一律**沿用各模块 `listVisible` 的角色可见范围**，不因搜索而放权——客户搜不到不属于自己的团单/他人订单，团主搜不到他人团单订单。搜索只是既有可见数据的检索入口，不是新的数据出口。

---

# Part C — 开发项 CHECKLIST

> **勾选状态的可信度**：`- [x]` / 标了「已落地/已完成」的可信；`- [ ]` **不一定代表没做**——
> 本清单从重构初期沿用至今，部分项做完后没回来打勾。**动工前先用 git log 与实际代码核对该项是否已落地**，
> 别看到空框就重做一遍。（校准 Part C/D 勾选状态本身是一件待办，见 `进度总览.md`。）
> 唯一确定**还没做**的是带 `🚧` 的项：目前是 `C-PROXY-ORDER`。
...
- [ ] **C-ENTRY-IA 功能入口编排落地**
  - 改动：按 Part B「B5」把入口位置固定；「我的」页移除与 NAV 重复的团单/客户订单/工作台入口；废弃 home 作为正式入口。
  - 判定：同一功能对所有有权限角色出现在同一固定位置；无重复入口；无权限角色隐藏。

- [ ] **C-AUTH-GATE 页面进入鉴权闸门统一化（落地 A13）**
  - 背景：现状各页各写——`groupOrder`/`my`/`my/info-edit`/`customerOrders`/`customerOrders/edit`/`setting` 未登录会导向 `pages/login/login`；而 `providers`/`operationLogs`/`userReview`/`tourGuides`/`dataCenter`/`message`/`profile` 等未登录**只把列表清空 + 显示一句受限文案，停在原页**，不导登录页（用户实测到的漏洞）；`sub-pages/groupOrder/detail` 则完全不判登录、仅凭 id 取数（属公开白名单，须保留但显式确认）。
  ...
  - **落地范围（已完成）**：`ebd7037` 收口 `requireLogin()`（`getRealProfile` 判定避开 DEV 预览、1s 去抖防 onLoad+onShow 叠登录页）并覆盖 `providers`/`operationLogs`/`userReview`/`tourGuides`/`dataCenter`/`message`/`profile` 七页；`662e8af` 再补 `sub-pages/groupOrder/add`（开团）、`sub-pages/groupOrder/productList`（本团商品）、选品页、商品表单页。
    > 2026-08-26 后续：`message` / `profile` / 选品页 / **整个商品库（商品库页、商品浏览页、商品表单页）** 都已整档删除（决策 5 / #8）；`groupOrder` 与 `customerOrders` 两个主 tab 补上了 `requireLogin`。
  - **不 gate 的例外**：`pages/feedback`（DEV-only 内部工具，`onLoad` 已挡 `!isDev`、不读业务数据）。
  - ~~商品库三页~~ **已整个删除**（B2 #8）。商品不是独立实体，只活在团单的 `productList` 里。
...
- ~~**C-PRODUCT 团主商品管理**~~ — **取消并删档（#8，2026-08-26）**。没有独立的商品管理页；商品在开团页内嵌新增、随团单存活。团单内的商品要改/移除，走「编辑团单」（`sub-pages/groupOrder/add`）与团单详情的「本团商品」（`sub-pages/groupOrder/productList`）。
- ~~**C-PRODUCT-LIB-SAVE 团单商品⇄商品库复用**~~ — **取消（#8，2026-08-26）**：根本没有商品库可以互通了。
...
- [ ] **C-ORDER 客户下单**：`pages/customerOrders/edit*`。从合法团单入口挑该团商品提交订单，记录 group order/customer principal/items/金额/状态/时间。判定：客户只见自己订单。
...
- [ ] **C-PROXY-ORDER 团主代客下单 + 订单认领（落地 A6「代下单订单的归属与认领」，OWNER 决策 14）**
  - 背景：现况**做不到代下单**。云端 `customerOrders.create` 的闸门是 `assertApprovedProfile(profile,['customer','owner','admin'])` 再加
    `if (!hasRole(profile,'customer') && !isOwnerOrAdmin(profile))` → 纯 `guide` 帐号直接被拒；且 `create` 写死
    `customerUserId: profile.id, customerOpenId: profile.openId`，订单会挂在团主自己名下，而 `canViewOrder` 对 customer 是比对
    `customerOpenId` → 被代下单的真客户**看不到自己的订单**。「帮谁下的」目前只有 `customerName`/`customerPhone` 两个文字栏。
  - 改动：
    - `cloudfunctions/businessData/resources/customerOrders.js`：`create` 放行本团归属的 `guide`（限自己管理的团单，走 `canManageGroupOrder`）；
      新增代下单分支——依手机号查 `users` 决定绑或留待认领；`canViewOrder` 让待认领订单对代下单团主可见。
    - `cloudfunctions/businessData/resources/users.js`：手机号绑定成功时触发认领（把该号码的待认领订单挂到该 principal）。
    - `cloudfunctions/businessData/lib/core.js`：认领判定 helper；对应的客户端镜像在 `services/auth/roleScope.js`（双通铁律，见 `DEVELOPMENT_GUIDE` §2）。
    - 画面与入口见 `PAGE_MAP.md` §5；栏位见 `FIELD_DICT.md` §5「待开发栏位——代客下单」。
  - **⚠️ 动工前必须先问 OWNER 的两题**（没答案不要动码）：
    1. **自动绑定的触发点**：绑定键只能是手机号，但微信登入拿不到手机号（`authLogin` 建号时 `phone: ''`），
       现况只能靠客户自己去「我的 → 账号资料」填 → 照字面写「登入后自动绑」会做出一个几乎不会触发的功能。
       要真能触发，得在登入或首次进订单页时主动要手机号——**要不要加这一步？**
    2. **防冒领**：现况手机号纯自填、只验格式（`users.js` 的 `/^1[3-9]\d{9}$/`），无所有权验证 →
       「填了号就自动认领」等于任何人填别人的号就能继承那笔订单（含已确认收款的）。
       建议自动绑定只接受微信 `getPhoneNumber`（全仓目前没用过）取得的验证号码，纯自填不触发——**采纳吗？**
  - 判定：团主在自己团单内代客下单，**收单截止后仍可下**；客户已注册 → 该客户登入后在「我的订单」看得到这笔；
    客户未注册 → 订单为待认领、团主端照常显示与操作，该客户日后完成手机号绑定后**自动**出现在他的订单列表；
    绑定前后都查得到代下单团主是谁（A7 操作记录有对应两笔）；冒领防线按上述第 2 题的结论生效。
...
- [ ] **D-AUTH-GATE**：清缓存/退登后逐一打开每个需登录页（团单/商品/客户订单/供应商/用户审核/操作记录/数据中心/消息/个人资料等）→ 全部导向登录页，登录成功回原页；无一页停在原页显示空/异常业务数据。无权/停用/过期账户 → 显示正式受限态而非登录页、也非空数据。公开白名单（团单详情客户视图 / 下单页）→ 未登录仅浏览态，管理操作不可见、后端拒绝。
...
- ~~**D-PRODUCT-LIB-SAVE**~~ — 随 C-PRODUCT-LIB-SAVE 一并取消（决策 6）。
- [ ] **D-GROUP**：开团表单 picker 校验与回填；本团商品快照；下架/停用商品不被新团单选用，历史保留。
- [ ] **D-GUIDE-ORDERVIEW**：团主看自己团单订单、改状态、确认收款；不得见他人团单订单。

## D5. 客户下单与付款

- [ ] **D-PROXY-ORDER**：① 团主在自己团单代客下单，填**已注册**客户的手机号 → 该客户登入后在「我的订单」看得到这笔，且订单显示代下单团主。
  ② 填**未注册**号码 → 订单为待认领，团主端可正常确认收款/取消；该号码的人日后登入并绑定手机号 → 该笔**自动**出现在他的订单列表，代下单痕迹仍在。
  ③ 团单**收单截止后**团主仍可代下单；同一时间客户自助下单要被挡。④ 负向：非本团团主不能代下单；用别人手机号不得认领到已确认收款的订单（防冒领，依 C-PROXY-ORDER 第 2 题结论）。
...
- [ ] **D-USER-DIRECTORY**：以 owner/admin 进入用户目录 → 看到全部非 owner 用户及真实 `roles[]`/状态/期限/申请状态；对一个**已审核**用户改角色（追加不覆盖、客户功能不掉）、停用后该账户业务全关、恢复后正常、设期限到期后团主功能关客户仍在；owner 不出现在可停用/降级项、admin 改 owner 被后端拒；local 与 cloud 目录/待审核口径一致；改完重开仍在且有操作记录。
